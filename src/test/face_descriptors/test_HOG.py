...
def test_compute_gradient_grayscale():
    ...
    # Since magnitudes are floats here, require isclose to test if they're
    # similar and check that every element in the array is similar enough
...
def test_block_histogram_creator_on_empty_input():
    assert np.array_equal(
        np.zeros(9), HOG.create_block_HOG_vector(np.array([]), np.array([])))
...
def test_HOG_vector_on_some_inputs():
    ...
    img[:sliceOfImg.shape[0], :sliceOfImg.shape[1],
        :sliceOfImg.shape[2]] = sliceOfImg
    expectedOut = np.float32(np.zeros(9*(256+64)))
    expectedOut[0] = 7.0
    # Calculated by hand with help from the gradient function
    expectedOut[3] = 2.98667816705
