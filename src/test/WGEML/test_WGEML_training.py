...
def test_getting_diff_matrix():
    ...
    assert (WGEML_training.get_graphs(x_view, y_view) == expectedOut).all()
...
def test_getting_D_1p_and_D_2p_with_two_neighbors_and_2_dims():
    ...
    out = WGEML_training.get_penalty_graph_2(
...
def test_getting_proper_top_2_eigenvectors():
    ...
    out = WGEML_training.get_top_d_eigenvectors(A, np.identity(3), 2)
...
def test_getting_U_and_w_for_a_basic_input_with_one_descriptor_and_dimension_10():
    ...
    out = WGEML_training.get_all_values_for_a_relationship(
    ...
    expected_U_0[0][1] = 0.70710678
    expected_U_0[1][1] = -0.70710678
